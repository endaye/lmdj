...
A fail-closed check is only as useful as what it tells the next reader. Six
Pull Requests over seven days did nothing but make existing release and Portal
...
error (#218), and surface exact-target audit detail (#227). Each was written as
a one-off repair of one message, so the next new gate repeated the omission.

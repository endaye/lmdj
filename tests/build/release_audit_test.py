...
class ReleaseAuditTest(unittest.TestCase):
    ...
    def active_entry(self) -> dict[str, object]:
        return {
            "tag": "lmdj-v1.0.22.0", "kind": "product", "identity": "1.0.22.0",
            "target_revision": TARGET, "channel": "canary", "disposition": "allocated",
            "profile": "web-runtime-host", "snapshot": "1.0.22.0",
            "evidence_paths": ["evidence.md"],
        }
    ...
    def install_static_authority(self, root: Path) -> None:
        paths = [
            ...
            ROOT / "apps/architecture-portal/versioned_metadata/version-1.0.22.0.json",
    ...
    def context(
        ...
        if not any(item.get("identity") == "1.0.22.0" for item in selected):
    ...
    def test_tracked_current_product_identity_is_locally_auditable(self) -> None:
        ...
        report = audit(context, remote=False, tag="lmdj-v1.0.22.0")
        self.assertEqual({item.code for item in report.findings}, {"ok"})

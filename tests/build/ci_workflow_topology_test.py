...
'''
    ...
    def test_package_lane_retains_its_comparison_evidence(self) -> None:
        """The archive dies with the runner; its digests must outlive it.

        Release assets are rebuilt and signed by `prepare` on the operator
        machine. Without the detached Manifest and checksum there is no
        digest-level cross-check between what CI proved packageable and what
        an operator later ships. This is comparison evidence, not a release
        input, so it is deliberately not the scope manifest that
        `tools/release/ci_evidence.py` reads.
        """
    ...
    def test_package_lane_states_its_fixture_and_dependency_assumption(self) -> None:
        ...
        Both are safe only because packaging runs the unit and component tiers
        against LFS-rehydrated fixtures. A generated fixture entering either
        tier invalidates that silently, so the assumption is stated where the
        next editor of the job will read it.
        """
        job = self.workflow_job("package")

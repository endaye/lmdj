...
`patch_id` currently has the form `{song_id}-{sha256(lanes.json + chart.mid)[:8]}`: only the suffix is content-derived. Current release evidence shows that both the Job-coupled full ID and the pipeline content suffix can vary across separate runs of the same audio. Do not claim cross-Job idempotency or repeatability until that release blocker is fixed and re-verified.

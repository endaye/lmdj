...
`CLAUDE.md` is the Claude-facing twin of this file — keep the two in sync when editing shared guidance.

The current end-to-end chain runs and has been exercised in the browser:

```text
browser submission → apps/api (idempotent POST /uploads) → single-slot FIFO JobExecutor
  → workers/audio (process_job) → selected runner:
      legacy: demo pipeline subprocess
      materials-v1: canonical Timing + four-stem separation + Material Extractor
  → packages/patchify
  → patch.json (lmdj.patch.v1) + samples written to the job dir
  → apps/web persists Job/submission identity, polls status/queue, and recovers after refresh
  → GET /patch + /files/* → loadPatch
  → fixed 16-pad Creator Workbench: Pattern / play / mute / keyboard / Web MIDI
  → Creator Export status + downloadable ZIP
Contract hub: packages/core-models (models + JSON Schema — the shared contract).
```

## Product model & active work

LMDJ's product code is designed around its own object model, **not** the demo's `Sample`/`LoopWindow` types: `Project / Patch / Pattern / Pad / Scene / Element / Render / Lineage`. The model dataclasses and the public `lmdj.patch.v1` JSON Schema live in `packages/core-models/`; the same package also owns the internal Worker → Patchify `lmdj.materials.v1` contract. `packages/patchify/` is a pure adapter depending on it. Patchify accepts either the legacy package (`samples/*.wav` + `chart.mid` + `lanes.json` + `report.json`) or a Material package (`materials.json` + `timing.json` + accepted samples), then emits the same stable `patch.json` consumed by Web, CLI, Worker, and API. Legacy mapping retains its semantic/reserved/empty layout. Material mapping uses the fixed Kick/Snare/Hat/Percussion/Bass/Melody/Vocal/Phrase A/B slots, preserves Empty decisions, and never asks consumers to read `materials.json`. Contract decisions are recorded in `docs/prd/decision-log.md`.

**Contract purity is the load-bearing invariant.** `lmdj.patch.v1` is the single contract shared four ways (Web / CLI / Audio Worker / API). The schema's single source of truth is `packages/core-models/lmdj_core_models/schemas/lmdj.patch.v1.schema.json`; the web copy under `apps/web/src/patch/schema/` and the generated `types.ts` are `npm run sync-contract` outputs, and `npm run check-contract` fails CI-style if they drift. **Consumers read only `patch.json`** — never `lanes.json` or `chart.mid` (the "truth is three-tier" decision, `docs/prd/decision-log.md` 2026-07-07). `patch_id` has the form `{song_id}-{sha256(lanes.json + chart.mid)[:8]}`. In the Worker/API path, random `job_id`, job directory, and status remain run-scoped. The Worker streams SHA-256 over the uploaded audio bytes and passes `source-<full-hex-digest>` as `song_id`; that same source ID is the package directory name. Always locate a package through `status.package_dir`, never by deriving it from the Job ID. Therefore the full `patch_id` is stable across Jobs for identical audio when the deterministic pipeline produces the same `lanes.json` and `chart.mid`; it is not a claim that arbitrary future pipeline variation is masked by source hashing.
...
- **`packages/core-models/`** — frozen Patch dataclasses + public `lmdj.patch.v1` and internal `lmdj.materials.v1` schemas. Material helpers enforce canonical serialization, package-root-safe relative paths, unique slots/IDs, exact 16-slot decisions, valid events, and A/B provenance.
- **`packages/patchify/`** — pure adapter with no DSP stack. Legacy packages keep their existing mapping. Material packages are schema-validated, mapped directly onto the fixed 16-role layout, and receive a deterministic `chart.mid`; both paths emit only `lmdj.patch.v1` to consumers.
- **`workers/audio/`** — audio → `patch.json`. `process_job` preserves queue metadata and the selected pipeline, writes every status transition atomically, and supports legacy `queued → separating → patchifying` plus stage-aware Material `queued → separating → extracting → patchifying`; all paths end in explicit `completed|failed`. The legacy runner remains subprocess-isolated. `CreatorPipelineRunner` composes canonical Timing, a registry/injected four-stem separator, quality-gated Material extraction, package writing, and Patchify without silent fallback. `separation/`, `pipeline_from_stems/`, and `benchmark/` retain their checkpoint, parity, cache, metric, and blind-listening responsibilities.
- **`apps/api/`** — FastAPI with file-backed submission identity and restart recovery. `POST /uploads` accepts an idempotency key; `GET /submissions/{id}` and `GET /queue` expose recovery/capacity; a single daemon worker drains an explicit FIFO queue with visible positions. Upload preflight enforces supported WAV/MP3 plus 200 MiB/600-second defaults. `LMDJ_PIPELINE=legacy|materials-v1` selects one runner when the app is built (default `legacy`), and that identity is persisted with each Job. Material/Separation manifests remain internal; only Patch assets and structured Export are public. This is still a single-process queue, not a distributed durable workflow. Successful temp uploads are not yet reclaimed.
- **`apps/web/`** — React 19 + Vite + TypeScript. `loadPatch` remains the single patch-loading entry point. The app persists submission/Job associations, restores them after refresh, displays filenames, capacity, FIFO position and `interrupted`/`extracting` honestly, and supports multiple uploads without losing the active Patch. The responsive Creator Workbench provides fixed 16-pad keyboard/Web MIDI play, Creator Export, and bidirectional exclusivity between a full-mix Phrase and ordinary material playback. Web never reads `materials.json`.

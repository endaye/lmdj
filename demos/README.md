...
- CI 默认将本目录归入 `docs_static`；`chameleon-lab` 和 `web-runtime-lab`
  另保留原有实验室测试通道及门户检查。自动化通过不代表物理硬件验证已经通过。
...
同日将 `apps/chameleon-lab/` 和 `apps/web-runtime-lab/` 迁入本目录。
根目录 `scripts/chameleon-lab.sh` 和 `scripts/web-runtime-lab.sh` 命令保持兼容。
`web-runtime-lab` 是 product-neutral experimental Host，不是 CF 上的
`apps/web-runtime-host`；目录迁移不改变物理 Touch-to-Sound 验收义务。

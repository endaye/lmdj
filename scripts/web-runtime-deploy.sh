...
# "<parent>/lmdj-web-runtime-deploy.XXXXXX/gnupg/S.gpg-agent" overflows it by a
...
owned_temp_parent_fits_gnupg_socket() {
  local parent="$1"
  local projected="$parent/lmdj-web-runtime-deploy.XXXXXX/gnupg/S.gpg-agent"
...
case "$command_name" in
  ...
  deploy)
    ...
    [[ "${LMDJ_ALLOW_RETIRED_NETLIFY_DEPLOY:-}" == "1" ]] || fail \
      "this Netlify deployment path is retired and its site is deleted; remedy: use scripts/cloudflare-host-deploy.sh TAG --target web-runtime-host"
    validate_tag "$1"

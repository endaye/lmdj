...
CANONICAL_PRODUCTION_URL = "https://lmdj-creator.netlify.app"
CANONICAL_NETLIFY_SITE = "lmdj-creator"
...
def read_tag_identity(root: Path, expected_product: str) -> tuple[str, str]:
    try:
        ...
        host = json.loads(
            (root / "apps/creator-web/module.json").read_text(encoding="utf-8")
        )
    ...
    if (
        ...
        or host.get("module") != "creator-web"
...
def _valid_http_evidence(
    ...
    *,
    ...
    expected_keys = {
        ...
        "index_sha256", "manifest_sha256", "product_build", "started_at",
        "status", "url",
    }
    ...
    return (
        ...
        and result.get("host_id") == "creator-web"
        ...
        and _valid_timestamp(result.get("started_at"))
        and _valid_timestamp(result.get("completed_at"))
        and str(result["started_at"]) <= str(result["completed_at"])
        and result.get("url") == (
            f"https://{deploy_id.lower()}--{CANONICAL_NETLIFY_SITE}.netlify.app/"
            if deploy_id is not None
            else CANONICAL_PRODUCTION_URL + "/"
        )
...
def _valid_success_evidence(document: dict[str, object]) -> bool:
    ...
    if (
        ...
        return False
    expected_archive = f"lmdj-creator-web-{host}-product-{product}.zip"
    if (
...
def validate_evidence_document(
    *, source: str, expected_contract: str
    ...
    if expected_contract == "lmdj.creator-web.deployment-evidence.v1":
        required = {
        ...
        if not _valid_success_evidence(document):
            raise DeployOrchestratorError("deployment evidence schema is invalid")
    elif expected_contract == "lmdj.creator-web.deployment-recovery-evidence.v1":
        required = {
...
def parse_arguments(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Creator Web deployment orchestration")
...
def run(options: argparse.Namespace) -> None:
    ...
    if options.command == "create-draft":
        draft = create_draft(
            ...
            host_version=options.host_version,
            tag=options.tag,
            site_id=require_environment("NETLIFY_CREATOR_SITE_ID"),
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(
            f"Creator Web deployment orchestration error: {redacted_message(error)}",
            file=sys.stderr,
        )
        return 2
    return 0

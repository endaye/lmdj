...
function renderSvg(source, sourceSha256) {
  ...
  <style>
    ...
    svg{background:var(--bg);font-family:Inter,"PingFang SC",system-ui,sans-serif}.page-title{font-size:28px;font-weight:750;fill:var(--text)}.page-summary{font-size:14px;fill:var(--muted)}
    .boundary rect{fill:var(--boundary);stroke:var(--border);stroke-dasharray:7 6}.boundary text{fill:var(--muted);font-size:13px;font-weight:700}.edge-mask{fill:none;stroke:var(--mask);stroke-width:10}.edge{fill:none;stroke:var(--line);stroke-width:2.5}.connection text{fill:var(--muted);font-size:12px;text-anchor:middle}.node-mask{fill:var(--mask)}.node{fill:var(--panel);stroke:var(--border);stroke-width:2}.kind-host .node,.kind-provider .node{stroke:var(--accent)}.node-title{fill:var(--text);font-size:17px;font-weight:750}.node-detail{fill:var(--muted);font-size:12px}.card rect{fill:var(--panel);stroke:var(--border)}.card-title{fill:var(--text);font-size:15px;font-weight:750}.card-body{color:var(--muted);font-size:13px;line-height:1.45}.marker{fill:var(--line)}
  </style>
...
function wrapStandaloneHtml(title, svg) {
  return `<!doctype html>\n<html lang="zh-Hans"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeXml(title)}</title><style>html,body{margin:0;background:#071011}main{min-width:760px}svg{display:block;width:100%;height:auto}.toolbar{position:fixed;right:16px;top:16px;z-index:2;display:flex;gap:6px}.toolbar button{border:1px solid #5a7377;border-radius:999px;background:#102023;color:#fff;padding:6px 10px;cursor:pointer}</style></head><body><div class="toolbar"><button data-theme="light">浅色</button><button data-theme="dark">深色</button></div><main>${svg}</main><script>for(const button of document.querySelectorAll('button'))button.addEventListener('click',()=>document.querySelector('svg').setAttribute('data-theme',button.dataset.theme));</script></body></html>\n`;
}

...
nlohmann::json manifest_json(
    std::string_view product = "1.0.15.2",
    std::string_view host = "1.1.2",
...
void test_digest_and_exact_identity_mismatches_fail_closed() {
  ...
  for (const auto& [manifest, digest] : {
           ...
           std::pair{canonical_manifest("1.0.15.2", "1.0.0"),
                     sha256(canonical_manifest("1.0.15.2", "1.0.0"))},
           std::pair{canonical_manifest("1.0.15.2", "1.1.2", 2),
                     sha256(canonical_manifest("1.0.15.2", "1.1.2", 2))},
       }) {
    ManifestGate gate;
    LMDJ_CHECK(

...
namespace {
std::optional<std::array<std::byte, 32>> digest(std::span<const std::byte> bytes) noexcept {
  std::array<std::byte, 32> result{};
  try {
    picosha2::hash256_one_by_one hasher;
    // PicoSHA2 copies each process range before reducing it. Bound that
    // temporary independently of the already-resident receive buffer.
    while (!bytes.empty()) {
      const auto count = std::min<std::size_t>(bytes.size(), 512);
      const auto* begin = reinterpret_cast<const unsigned char*>(bytes.data());
      hasher.process(begin, begin + count);
...
TransferTransactionResult TransferReceiver::commit(std::uint32_t request_id) noexcept {
  ...
  const auto actual = digest(staging_);
  if (!actual) {
    clear();
    return TransferTransactionResult::resource_limit;
  }
  if (*actual != identity_.sha256) {
    clear();

...
- 影响：`apps/creator-web/src/app.tsx` 的 `activateAudio` 在所有拒绝或失败
  路径恢复尝试前 phase，并继续单独报告非 `TypeError` 错误；reducer 仅在
  当前 phase 仍为 `activating` 时接受恢复动作，因此尝试期间更新的 Runtime
  发布始终优先；`status_bar.tsx` 的 Activate
  按钮在 handler 缺失或 `audioActivationReady !== true`（Host 未确认停在

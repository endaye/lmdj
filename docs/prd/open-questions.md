...
| Provider SDK 的 Artifact 字节访问由谁提供：输入 resolver 与输出访问口分别落在哪一层？ | 已批准的 `lmdj.capability.v2` 能用显式端口确定声明的 Schema 身份，但 `ArtifactRef` 不携带 Schema provenance，AttemptStore 也没有输入 Artifact resolver，不能把端口身份校验伪装成字节级 Schema 校验。2026-08-16 的 analysis-bench 原型进一步确认这个缺口是**双向**的：输出侧 SDK 同样没有读取已提交 Artifact bytes 的访问口，Host 只能按 `.lmdj-workspace/attempts/` 私有磁盘布局重建路径；原型的 Host 注入桥接是临时方案，不得直接毕业为正式接口（见 [decision-log](decision-log.md) 2026-08-16 条目）。 | 首个需要解析结构化 Artifact bytes 的正式 Capability 实现前，单独设计输入 resolver、输出字节访问口与验证器边界。 | 待架构设计 |
...
| 长素材（如 60 秒 Loop）采用什么 prepared-PCM 资源模型：每 Pad 均匀上限改为 Bank 内共享配额，还是抬高固定堆？ | Stage 8B 设计评审（S8B-D10）确认：现行每 Pad 240,000 帧上限源自 64/128 MiB prepared-PCM 预算，60 秒素材在均匀模型下需要 369 MB/Bank，超过 512 MiB 固定堆；而共享配额可在不抬堆的前提下支持单 Pad ≈60 秒、全 Bank 合计 ≈174 秒立体声。该变更波及 Cooker Bank 分配、audio-runtime PreparedSampleBank 布局（lock-free 发布路径）、manifest 语义、Facade 校验与"他人占用配额"新失败类别。抬 1 GiB 堆被 iPadOS 单页内存限制排除。 | 与上方 Loop 素材 BPM Time-stretch 问题同一次设计评审（长素材大多是跟 BPM 的 Loop）。 | 待设计评审 |

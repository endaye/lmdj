...
**Architecture:** 薄层（spec 方案 A）：`executor.py`（后台单槽执行器，`threading.Lock` 串行 `process_job`）+ `app.py`（`create_app` 工厂 + 路由，依赖注入 runner/jobs_root）。复用 `lmdj-audio-worker` 的 `process_job`/`read_status` 与文件系统 job 库。设计依据：`docs/superpowers/specs/2026-07-09-lmdj-app-api-design.md`。

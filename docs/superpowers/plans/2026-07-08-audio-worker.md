...
**Architecture:** 三模块分层（spec 方案 A）：`status.py`（JobStatus + 原子读写）、`runner.py`（`PipelineRunner` 协议 + 子进程 `DemoPipelineRunner`）、`job.py`（`process_job` 编排）+ `cli.py`。设计依据：`docs/superpowers/specs/2026-07-08-lmdj-audio-worker-design.md`。

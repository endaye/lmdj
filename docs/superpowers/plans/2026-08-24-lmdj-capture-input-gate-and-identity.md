...
package-lock.json); Product Build **1.0.34.0** (products/lmdj/version.json
build 33 → 34). Regenerate with repo tooling
...
a SECOND commit: `scripts/architecture-portal.sh version 1.0.34.0 canary`
...
- Product Build **1.0.34.0** is allocated for the new Assembly composition,
  with its immutable Architecture Portal snapshot
  (`scripts/architecture-portal.sh version 1.0.34.0 canary`).
...
- The version-carrying portal pages change with the new Product Build, and
  the `1.0.34.0` snapshot is frozen in the same Task (second commit).

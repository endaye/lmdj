...
**Architecture:** 新增 `src/api/client.ts`（纯 fetch，无 React，mock fetch 可测）；`App.tsx` 状态机加 `uploading` 态 + landing 第三入口；三条加载路（拖目录/示例/API）汇成 `Map<path, ArrayBuffer>` 复用同一 `loadPatch`。设计依据：`docs/superpowers/specs/2026-07-10-lmdj-web-api-integration-design.md`。

...
Implementation affects `audio-runtime`, observed `5.0.0` / API `2` at the baseline. A1/A2 are staged source changes, not independently published packages. Adding engine storage changes the public C++ object's layout: classify that as an ABI break requiring the next Audio Runtime MAJOR and a coherent static-consumer rebuild, not a Minor promise. Direct manifest consumers at this baseline are `application-facade` and `web-runtime-platform`; the complete consumer closure must be checked at allocation. Keep current manifests unchanged in these source slices and explicitly report the staged ABI boundary in their PRs/current module page. Before any independent package, Product Build or team-test distribution containing them, allocate coherent identities from the then-current active manifests in the #1230 assembly integration step; never reuse an old-ABI binary or guess an available version here.

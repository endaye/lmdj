...
| --- | --- | --- |
| D01 Global transport — responsibilities and six transitions approved | Separate global Play/Stop and current-Pattern Pad-action Record keys with the coupled §7 state table; page navigation does not redefine/stop them | No combined-state approval blocker remains. Restart/resume/count-in timing is not invented. Current source separation is an implementation capability dependency, not a product redefinition or proof of implementation. |
...
- Lock the bounded slice's still-unshown control/state rules: Pad selection versus triggering, applicable encoder/direction behavior, Back/draft ownership, System and layout-selector/fallback access and preference location. D01's six transitions are approved; only separate timing details remain unspecified. Confirm or explicitly defer inapplicable D02–D08 details; do not require all future screen content to be designed or implement unresolved semantics.

...
Date: 2026-09-11. Status: design DRAFT with narrow shown-preview approval (“我觉得没有问题👍”), recorded in the [preview approval](https://github.com/endaye/lmdj/issues/1214#issuecomment-5634900788), and the subsequent fully confirmed six global transport transitions in §7: separate Pattern Play/Stop and Pad-action Record keys with coupled states. Unrelated D02–D08 and keyboard/focus behavior are not thereby approved. U0 remains incomplete. Full migration-plan execution is authorized subject to its applicable U0 gates.
...
| Q07 Record | Current Q/A `recordSequence` → `beginSequenceJourney` opens event journal; requires running audio/session/Project and awaits settings tail | Approved global Record: from stopped starts playback and recording; from playing starts recording; while recording ends recording and keeps playing | Records Pad actions into current Pattern for sequencing/overdub, never microphone/master WAV. Current UI disables repeated Record; replace that source behavior through the §7 dependency and approved state table. End recording commits/retains recorded events. |
| Q08 Stop (current journal operation) | Current Q/A `stopSequence` → `stopSequenceJourney` flushes/closes journal, not general playback Stop | Reuse journal closure for Record-off and for Play/Stop while recording; the latter must also stop playback through separate playback authority | Current journal recording/switch → flushing → stopped receipt; update/show committed revision only when a Project exists and receipt `committedRevision` is non-null; otherwise retain revision and inspect authority. Preserve failure/recovery handling and recorded events; neither optimistic feedback nor failure may silently discard them. |
...
The journey below describes existing journal operations and their acceptance evidence, not an implemented global transport. The approved six transitions in §7 require its capability dependency; audio activation is not the target Play action. In the target, Record from stopped starts both playback and recording; ending recording via Record keeps playback running, whereas Play/Stop ends both and retains recorded events.
...
Play/Stop and Record are separate physical global keys with coupled states. Record records Pad actions into the current Pattern for sequencing/overdub, not master audio. Normal page navigation neither redefines their duties nor stops global transport. This supersedes independent-state, disabled Play, touch-only Stop and mode-dependent Record proposals. Sample microphone capture, Perform/master recording, auditions and saved replay retain separately named actions.

The user explicitly confirmed this complete table in this conversation, including the final “对的”; all six transitions are approved, not pending proposals:

| Current state | Play/Stop press | Record press |
| --- | --- | --- |
| stopped | playing, not recording | playing and recording |
| playing only | stopped | playing and recording |
| playing and recording | stop both playback and recording | stop recording, keep playing |

Ending recording commits/retains recorded events. There is no recording-only steady state or arm-and-wait proposal. Busy, error and recovery feedback must follow actual receipts, not optimistic success, and failures must not silently drop events. The user also confirmed that every stopped-to-running start begins the current Pattern at its beginning, without count-in; toggling Record while playing does not restart playback. These timing rules are settled.
...
| Sequence settings | BPM draft, 1 BPM | Swing draft, 1% | New Pattern bars 1/2/4/8 only in creation stage | Unassigned | Directions move T focus; −+ adjusts focused value. Separate global Record and Play/Stop keys follow the coupled §7 state table. |
...
| --- | --- | --- |
| D01 Global transport — responsibilities, six transitions and timing approved | Separate global Play/Stop and current-Pattern Pad-action Record keys with the coupled §7 state table; page navigation does not redefine/stop them | Starts from stopped begin the current Pattern at its beginning without count-in; Record toggles while playing preserve playback phase. Current source separation is an implementation capability dependency, not a product redefinition or proof of implementation. |
...
These historical prototypes do not implement the subsequent approved six-transition global transport table (§7). Their Record/Stop labels, touch-only routes and disabled repeat Record controls document earlier simulations, not implementation or validation of that approved table.
...
- Lock the bounded slice's still-unshown control/state rules: Pad selection versus triggering, applicable encoder/direction behavior, Back/draft ownership, System and layout-selector/fallback access and preference location. D01's six transitions and timing are approved. Confirm or explicitly defer inapplicable D02–D08 details; do not require all future screen content to be designed or implement unresolved semantics.
...
Manual inventory review covered six `CreatorMode` members, global StatusBar/ErrorPanel actions, all listed action-bearing components, source handlers, current guards and source/draft/receipt distinctions. Remaining decisions are D02–D08, not D01's approved transitions or timing; this is coverage of the design audit, not runtime acceptance.

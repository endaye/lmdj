...
| G02 Mode Rail: Sample | M/A; active Sequence recording/switch must successfully stop before Sample entry; flushing prevents transition | L Sample | Stop failure keeps current mode and authority error. Capture trim overlay is a separate path, not this mode switch. |
...
| G04 Mode Rail: Perform | M/A controller + configured capture + ready Project; actual record uses stricter `canRecord()` | L Perform | On leaving Perform await `controller.leave()`; failure does not navigate. Do not equate visible Perform with record-ready. |
...
| Q07 Record | Q transport → A `recordSequence` → `beginSequenceJourney` | L Record and labelled T Record Sequence equivalent | Requires running audio/session/Project, awaits settings tail. Records Pattern events; not microphone or stereo performance. Active/switch/flushing disables repeated Record. Error code is shown, no false recording indicator. |
| Q08 Stop | Q transport → A `stopSequence` → `stopSequenceJourney` | T Stop; physical Play-key alternative remains D01 | Recording/switch → flushing → stopped receipt; update/show committed revision only when a Project exists and the receipt's `committedRevision` is non-null; otherwise retain the current revision and inspect authority. Flushing disables Stop/relevant edits. Failure invokes authority refresh; never navigate away as if saved. |
...
| F05 Record Performance; Flush Performance; Stop Performance | F controller `record/flush/stop`, `canRecord` | T recording page; L Record only with labelled context | Record requires audio running, OPFS, matching capability/capture readiness and idle controller. Starting/recording/flushing/stopping distinct. Flush is not final Stop; failures surface with take state intact as controller permits. |
...
| F08 Replay saved Performance; Stop Replay | F `beginReplay`, `stopReplay`, `refreshReplay` | T saved performances; L Play proposal only for explicit selected replay | Observe playing/neutral/resolved revision. Stop waits for neutral confirmation, with bounded status queries and timeout. Leaving must not silently claim stop success. |
...
| Context | K1 | K2 | K3 | K4 | Direction / −+ / Record / Play proposal |
| --- | --- | --- | --- | --- | --- |
| Project browse | Unassigned | Unassigned | Unassigned | Unassigned | Directions move list focus, T Open explicit. −+ no action unless numeric control focused. Record/Play unavailable with reason. |
| Sequence settings | BPM draft, 1 BPM | Swing draft, 1% | New Pattern bars 1/2/4/8 only in creation stage | Unassigned | Directions move T focus; −+ adjusts focused numeric value; Record calls Q07. Keep T Stop Q08. Play-key semantics D01 unresolved. |
| Sample edit | Start frame | End frame | Volume 0.1dB | Viewport zoom, UI only | Directions navigate unless a waveform/range owns arrows. Record opens labelled capture flow; no direct overwrite. Play audition target/stop policy needs D01. |
| Capture trim overlay | Selection start | Selection end | Unassigned | Unassigned | Capture modal owns focus and Record meaning. T Commit/Discard/Close explicit. Sequence transport must not steal capture actions. |
| Perform FX | Filter / Gate | Delay / Reverse | Reverb / Crush | Stutter / Cutter | T names the selected FX page. Record Performance; Play only selected saved replay if approved; Stop paths remain explicit. −+ and keys preserve gesture release. |
...
| --- | --- | --- |
| D01 Transport contexts | Retain named T actions; enable Record only for explicit current workflow; leave unsupported Play disabled | Sequence has Record/Stop but no standalone Play button in current surface. Shared Play=Stop toggle, Sample preview target and replay precedence change semantics. No generic transport mapping may enter implementation yet. |
...
| D06 Event-grid target and projection dependency | Preserve a real read-only Sequence event grid as the desired final outcome; verify its Facade projection and record a dependent capability Task if absent | Define verified event data and grid scope without assuming the example's 8 tracks/64 steps are limits. A provisional text/metadata slice cannot silently redefine the final UI. Missing projection is a dependency, not grounds to remove the grid; standalone playback is a separate capability question. |

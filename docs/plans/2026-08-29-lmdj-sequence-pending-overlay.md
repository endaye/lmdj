...
> **For agentic workers:** Follow repository `AGENTS.md` to execute this plan in one reviewable Task. Do not split the Issue across commits.
...
- [x] **Step 6c: Integrate pending overlays with authoritative switches**

  Keep ordinary different-Pattern overlap rejected, but let a validated switch or Stop replace only the exact pending generation, Pattern identity, and activation frame returned by Audio Runtime. Retain acknowledged switch authority until its exactly-once Facade flush completes. A Stop before that boundary atomically replaces the target with clean committed old-Pattern truth; if clean publication fails, cancel the exact unclaimed target so command replay can recover, and quiesce/clear when audio has already claimed it. Reject BPM changes while a switch owns a boundary before calling the Facade, while allowing a BPM/settings publication immediately followed by an authoritative switch. Cover the production ControlRuntime path, shared Runtime Session stale-boundary suppression, and the packaged browser journey.

- [x] **Step 6d: Conserve cancellation and make Stop replay boundary-fresh**

  After exact target cancellation, derive every clean Stop publication attempt from current transport rather than retaining the superseded switch frame; an exact durable replay after that frame has elapsed receives a fresh next-Bar activation without a second Project mutation. Interpret the claimed marker before matching an audio-owned generation, including a deterministic gate after the apply-point CAS, so exact switch authority remains valid while ordinary overlap stays rejected. Count every successful queued or audio-owned cancellation exactly once, never count its later callback reclamation as supersession, and enforce `accepted = applied + superseded + canceled + pending` in component and concurrent stress tests. Keep all deterministic hooks private to testable archives and extend the production source-boundary marker gate.

- [x] **Step 6e: Close dual-pending, Stop accounting, and cancellation TOCTOU**

  Expose `pending_publications` as the number of distinct queued and audio-owned generations, not a boolean inferred from the latest authority, and prove conservation while A is audio-owned and B is queued. During quiescent `stop()`, exchange both generation atomics, strip claimed markers, mark their slots reclaimable, and count each distinct live generation canceled exactly once; keep cumulative Pattern publication counters across `start()`. In Web Control, deterministically pause after the first current-generation query, let the target apply and clear pending, then require the no-pending path to revalidate current generation and fail closed. Gate the interleaving hook to the private testable Web archive and extend production symbol/marker exclusion.

...
**Spec:** `docs/design/2026-07-15-multi-separator-benchmark-design.md` §7（manifest）、§8（执行/缓存/输出结构）、§4/§10（orchestrator/错误模型）。
...
```text
| #10 | Separation Phase 1C-a：benchmark 执行层——manifest schema/loader、ffmpeg 输入归一化、orchestrator（全链 + 六元组缓存 key + resume + 单组合失败不中断）、`dev.sh bench` CLI；suno mini-run 真实验收。plan: `docs/plans/2026-07-16-separation-phase1c-exec.md` | `workers/audio/benchmark`、`testdata/` |
```

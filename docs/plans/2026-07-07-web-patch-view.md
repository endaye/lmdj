...
**Architecture:** 三层分离：`src/patch/`（契约层：schema 拷贝 + codegen 类型 + loader）、`src/engine/`（裸 WebAudio，lookahead 调度，不依赖 React）、`src/ui/`（React 组件）。设计依据：`docs/design/2026-07-07-lmdj-web-patch-view-design.md`。
...
`docs/design/2026-07-07-lmdj-web-patch-view-design.md`。

...
Design: [approved migration design](../design/2026-09-08-cloudflare-migration-design.md).
...
Status: active implementation. See the current-state addendum in the design;
account access and three fixed deployments are established, while PR Preview
and full recovery/retirement acceptance remain incomplete.

## Execution order

The approved order was T1–T4 before Portal production cutover. The separately
authorized fixed-site deployment in PR #905 ran ahead of complete T3 acceptance;
this does not waive T3 or the remaining T4 recovery proof. T5–T7 cover both product Hosts;
...
## Current task graph and this documentation Task

Relates to #921 and #873. #921 owns design/capacity; #922 owns exact-head
Preview; #923 owns Portal recovery/governance; #924 owns the shared Host
adapter; #925 and #926 own Creator and diagnostic Runtime acceptance; #927
owns retirement. Dependencies are #921 → #922 → #923, #921 → #924 →
#925/#926, then #922/#923/#925/#926 → #927. The capacity measurement part
of #921 consumes #922 pilot results: design availability unlocks a bounded
pilot, not broad activation. This feedback edge must not deadlock the pilot.

This Task declares only this plan, the migration design and its sanitized
seven-day audit JSON. It imports the approved historical record, reconciles
current evidence and leaves unverified budget/retention decisions explicit.
No production code or current Portal page is changed. Validation: local links,
JSON parsing, staged ownership and Portal check for documented source facts.

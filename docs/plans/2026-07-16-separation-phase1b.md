...
**Spec:** `docs/design/2026-07-15-multi-separator-benchmark-design.md`（§5/§6/§10；§6 对 RoFormer 的门禁：只启用能追溯来源、固定 revision、明确四轨与许可的 checkpoint）。
...
```text
| #9 | Separation Phase 1B：BS-RoFormer（SDR 9.65）+ Mel-Band RoFormer（SDR 8.22）四轨 runner 与 experimental registry 条目；MSST 家族共享实现抽取（scnet 薄壳化）；Mac MPS 验收。plan: `docs/plans/2026-07-16-separation-phase1b.md` | `workers/audio/separation/runners`、`config/` |
```

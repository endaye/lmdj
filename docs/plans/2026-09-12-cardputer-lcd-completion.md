...
1. Prerequisite Pad feedback correction: processed current-epoch receipts,
...
declared paths, Build allocation and immutable snapshot. The unconnected
...
The integration Task must allocate the completed Host/Assembly candidate and
include its immutable snapshot before formal A1. Existing snapshots stay frozen.

...
## Generated-document preflight follow-up

Current-head review `5190771227`, run `34758763563/1`, identified a real
producer/transport mismatch: a 10000-digit tag passes the identity regex but
produces a 20841-character body. The actual transport rejects it before HTTP,
after the controller has saved `create_intent=True`, causing an unnecessary
permanent `unknown-create`. Independent review reproduced the same outcome.

Share the existing pure POST document predicate between producer and transport,
and validate generated bytes before opening/enrolling the journal. No bound is
widened, no remote write is retried and operation identity remains controller
authority. The focused regression failed before the repair with no expected
local refusal; a companion boundary fixture must flow the largest generated
body within the existing transport bound through the real GitHubClient and
observe its POST plus final verified far-side state. This proves the internal
transport contract, not live GitHub acceptance of artificial long versions.

Verification: focused pre-fix regression exit 1 (`EvidencePrError not raised`);
after repair, PR controller 36/36 (0.098 seconds), GitHub API 39/39
(0.012 seconds), and `git diff --check` exit 0. Portal content is unchanged from
the previously verified Task; this amendment changes only local POST preflight.

Other review dispositions: distinct duplicate PRs are rejected intentionally;
`closefd=False` leaves the explicit finally block as sole FD owner; marker-only
state cannot safely be distinguished from lost post-effect state and requires
manual reconciliation. Failed private temporary files may remain for diagnosis;
cleanup is not automatic and no returned success depends on them. The low-level
transport limits routes/payloads while the trusted controller binds operation,
numeric PR, exact head and authorization; it is not itself a merge gate.

...
The Architecture Portal snapshot command requires a clean worktree and records the committed source revision. The Controller originally committed implementation/current Portal truth as `129f6908e932a7a840f5f52f6f5ba4b068b7ebd2` and froze `1.0.26.0` at descendant `20650c0deb8292639791d80983c0559f2da7447c`. After protected-main PR #260 changed a projected current Portal page, that unmerged snapshot was removed in source-preparation commit `d6bffdee545abba46921cc146e11b7e8739980b8` and regenerated through `scripts/architecture-portal.sh version 1.0.26.0 canary` in snapshot commit `6bd3b59faf4c0770c6a790ea93a983860d9196aa`. The regenerated metadata binds source revision `d6bffdee545abba46921cc146e11b7e8739980b8` and its exact projection; the superseded branch-local snapshot never reaches the final Pull Request tree.

The earlier local correction `1aa70b27ca1611ee23b8493617d6da46599b7a83` only aligns the source-package test expectation with the planned local-proof Provider version `1.0.5`; the complete implementation at that revision passed `scripts/core.sh test dev full` with `73/73` tests. These provenance commits remain in the reviewed branch ancestry, but this Pull Request does not register a `1.0.26.0` Release intent.
...
- Modify: `docs/plans/2026-08-22-lmdj-host-settings-flock-crash-recovery.md`

...
- The approved design is `docs/design/2026-08-11-risk-based-ci-gating-design.md`; implementation must not weaken its fail-closed rules.
...
The final-review closure also requires NUL-safe `git ls-files` coverage proving every current tracked path matches explicit ownership or an explicit full rule. Manifest validation checks exact changed-file keys/results/path counts/canonical paths/no duplicates and enforces `full` = every lane, `draft` = the exact two draft lanes, and focused lanes = the exact normal Ready/no-label ownership union recomputed from the manifest inventory. Focused manifests reject unknown/unclassified/full-rule paths and the three-expensive-family trigger. The classifier and validator share this evaluator so their semantics cannot drift. It removes all expensive-family exemptions; uses exact Portal workflow names; gives `scripts/core.sh` complete Core + Package; closes all six current Core concurrency/stress sources (including the two exact Project I/O tests), public helper/registration and version/package inheritance; assigns Web audio/project-I/O fixtures to their actual consumers; and splits Web-only MJS/source-boundary inputs from native `web-runtime-platform` inputs. Future unknown `scripts/**`, `tests/**`, `.github/**`, packages, or workflows remain unclassified and therefore full.
...
- focused manifest path mutations: require Gate rejection when a Core path is downgraded to Docs, a Web path omits Portal, or a central/full-rule path is encoded as focused.

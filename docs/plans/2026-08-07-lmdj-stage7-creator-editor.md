...
- The approved design is `docs/design/2026-08-07-lmdj-stage7-creator-editor-design.md`. A conflict returns to design review; implementation must not silently weaken it.
...
After Task 14, inspect every commit/file and use the repository review workflow before requesting push authorization. A Stage 7 PR must declare the Documentation Impact block above, depend on merged PR #94, and require Core CI, Web Runtime Host, Creator Web, and Architecture Portal checks.

...
| --- | --- | --- |
| M2 / #373 | RED: the real `SIGKILL` restart journey found a candidate without the two acknowledged unflushed events, while the Project I/O test could not compile without durable-tail API/metadata. Review RED: after F1 append/execute failure, a successful cumulative F2 left F1 recoverable; apply could double-mutate and overwrite F2's newer same-key event. Complete-line checksum/sequence/canonical corruption also returned generic path/detail. GREEN: focused 2/2 passes with F2 resolving covered earlier flushes, recovery containing only the later truly uncommitted tail, one apply revision, no second apply/discard mutation, and the newer same-key value preserved. Torn and complete-line corruption retain exact bytes and return uniform path/offset/prefix/length/stable-reason/repair-or-discard evidence. Fresh review-fix verification passes Core fast 38/38, full 79/79, stress 4/4, Portal 59/59 with 37 pages/10 diagram sources/20 outputs/42 routes, plus dependency, active-tree, version and production hook-symbol gates. | This is functional source evidence only. #379 owns accumulated Module/Host/Product identity and complete integrated automated acceptance; #380 owns the clean exact-main immutable snapshot. The five #360 physical/manual rows above remain `deferred / unverified`. |

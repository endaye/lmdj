...
| A3 | Reshape the published artifacts: ship the Build Manifest as a detached sibling Release asset and keep the archive payload-only | triage A3, **decided 2026-08-24** in [`../prd/decisions/2026-08-24-build-manifest-detached.md`](../prd/decisions/2026-08-24-build-manifest-detached.md) ([#211](https://github.com/endaye/lmdj/issues/211)) | Contract and fields unchanged (`build_time` stays, version-management.md §4); touches `scripts/package-core.py`, the `tools/release/` asset inventory, `packaging/core/README.md`, `tests/distribution/` and `tests/build/`, and the pipeline spec's asset list; due before the first external distribution |

...
| C9 | Shard or shrink `project_io.project_store`, the next test sitting on its budget | found while merging C8, 2026-08-19 | `TIER component` = 30s, and the **coverage** build gets no sanitizer multiplier, so 30s is the hard limit there. Measured on `main`: **25.51s at `93b7d3f2`** (85% of budget) and 23.58s at `3df66c74`; it then timed out at 30.10s on PR #190, whose diff is one threshold value and documentation and cannot affect runtime. Same disease as C8 in a different test. C8's seed-sharding is the template where the workload divides; check first whether this one's cost is one shared setup, as C8's turned out to be |
| C10 | Split `facade.application` itself — C7 relieved the additions, not the file | found 2026-08-19 | C7 moved the new failure-contract tests out, and that binary is now 4.76s. But `facade.application` still measures **61.31s of its 90s ASan budget on green `main`** (68%), and timed out at 90.71s on PR #191, whose diff touches only domain tests. One ~3,400-line binary with ~25 tests still shares one budget. Split by surface (project / sample / render). Also remove the orphaned comment blocks #188 left behind in that file when its tests moved out |

...
| C7 | Split `tests/core/facade/application_test.cpp` before it grows further | found during C6, 2026-08-19 | On `main` it already runs **47.76s of its 90s ASan budget** (component tier 30s × ASan 3×), in a single 3,900-line binary. Nothing is wrong with it today, but it has one budget for ~30 tests, so the next few additions decide whether the whole file starts timing out on a contended runner. Split by surface (project / sample / render) so each carries its own budget |

...
| ~~E2~~ | ~~Add a range parameter to `CaptureBuffer.envelope(bins)`~~ | triage E2 | **implemented in the Product Build `1.0.28.0` candidate by [#212](https://github.com/endaye/lmdj/issues/212)**. Recording requests the complete buffer; trimming and commit retry request the live selection. Exact bins visit only that range, summaries cover only full blocks, partial edges scan exact samples, and the cache keys both range values |
...
6. ~~**E2**~~ — implemented with real selection zoom in the `1.0.28.0`
   candidate. **E3** starts only with the approved pre-commit Crop feature.

...
| T1 | `open — 1.0.20.0 must restart at step 1` |
...
T1 状态：`开放 — 必须在 1.0.20.0 从步骤 1 重新执行`。不得沿用
`1.0.18.0` 的步骤 1–6，也不得把自动化事件当成实体键盘、实体 MIDI 或听感通过。
...
| 步骤 | 操作与必须观察的结果 | 人工结果 |
| ---: | --- | --- |
| 1 | 打开 Creator；页面正常加载，未自行播放声音。 | `未执行` |
| 2 | 导入正式 `.lmdj` 工程；短暂 `importing` 后回到 `ready`，操作按钮恢复。 | `未执行` |
| 3 | 记录 Project ID、revision、BPM 和 64 Pad 占用，确认数据符合测试工程。 | `未执行` |
| 4 | 点击 **Activate Audio**，确认音频成功激活。 | `未执行` |
| 5 | Bank A 逐项验证实体键盘：`Q..I -> A1..A8`，`A..K -> A9..A16`；同时确认每个 Pad 显示相同键帽且能听到对应声音。 | `未执行` |
| 6 | 切换 Bank B、C、D，抽查同一局部键序映射、Pad 地址和声音对应。 | `未执行` |
| 7 | 点击 **Suspend**，确认停止；再次点击 **Activate Audio**，确认恢复触发。 | `未执行` |
| 8 | 刷新页面，重新打开同一 Project，再次显式激活音频。 | `未执行` |
| 9 | 确认无重复、漏触发、按下卡住或自动播放，且 Project 数据未丢失。 | `未执行` |
| 10 | 点击 **Export report**，检查隐私安全字段并记录文件 SHA-256。 | `未执行` |

### 必须单独记录

| 观察项 | 人工结果 |
| --- | --- |
| Import 是否退出 `importing` 且按钮恢复 | `延期 / 未验证` |
| 实体键盘空间顺序与逐键听感 | `延期 / 未验证` |
| 实体 MIDI | `延期 / 未验证` |
| Bank B/C/D 地址与声音对应 | `延期 / 未验证` |
| Suspend 后再次激活 | `延期 / 未验证` |
| macOS Safari | `延期 / 未验证` |
| iPadOS Safari touch / lifecycle | `延期 / 未验证` |
| 报告检查、SHA-256 与签字 | `延期 / 未验证` |
